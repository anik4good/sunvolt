...
export function Footer({ businessName, phone, whatsapp, address, d }: FooterProps) {
  return (
    <footer className="bg-navy text-white">
      <div className="mx-auto grid max-w-6xl gap-8 px-4 py-10 sm:grid-cols-2 lg:grid-cols-3">
        <div>
          ...
          <p className="mt-3 text-sm text-white/70">{d.footer.tagline}</p>
        </div>

        <nav aria-label={d.footer.quickLinks}>
          <h3 className="text-sm font-semibold text-solar">{d.footer.quickLinks}</h3>
        ...
        <div>
          <h3 className="text-sm font-semibold text-solar">{d.footer.contactH}</h3>
          <ul className="mt-3 space-y-2 text-sm text-white/80">
            <li>
      ...
      <div className="mx-auto max-w-6xl px-4 py-4 text-center text-xs text-white/60">
        © {new Date().getFullYear()} {businessName}. {d.footer.copyright}
      </div>
    </footer>
  );
}
