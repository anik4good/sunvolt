...
function StockBadge({ stock }: { stock: number }) {
  ...
  if (stock < 5) {
    return <Badge variant="outline" className="border-solar text-solar-dark dark:text-solar text-[10px] px-2 py-0.5">Low · {stock}</Badge>;
  }
  ...
  const columns: ColumnDef<Product>[] = React.useMemo(() => [
            ...
            <div className="min-w-0 max-w-[180px]">
              <div className="flex items-center gap-1">
                <Link
                  prefetch={false}
                  href={`/admin/products/${product.id}`}
                  className="truncate font-semibold text-foreground hover:underline text-xs"
                >
                  {product.name}
                </Link>
                {product.featured && (
                  <Star className="size-3 fill-solar-dark text-solar-dark dark:text-solar shrink-0" aria-hidden />
                )}
              </div>
    ...
    // Category column
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <Badge
            variant="outline"
            className={product.category === "package" ? "border-solar text-solar-dark dark:text-solar text-[10px] px-2 py-0.5" : "border-navy/30 text-foreground text-[10px] px-2 py-0.5"}
          >
    ...
    // Warranty column (hidden by default)
    {
      ...
      cell: ({ row }) => {
        ...
        return warranty ? (
          <Badge variant="outline" className="border-navy/30 text-foreground text-[10px] px-2 py-0.5">
